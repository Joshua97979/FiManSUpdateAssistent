...
public class Model {
	...
	private boolean handlePathToUpdates(OptionPanel optionPanel) {
		...
		if (configFile.getPathToUpdates().equals(optionPanel.textFieldPathToUpdates.getText())) return true;
		try {
			initUpdates(optionPanel.textFieldPathToUpdates.getText(), false);
		} catch (FileNotFoundException e) {
			controller.showMessageDialog("Keine Datei an angegebenem Pfad gefunden!");
			return false;
		} catch (XmlFormatException e) {
		...
		try {
			configFile.setPathToUpdates(optionPanel.textFieldPathToUpdates.getText());
		} catch (FileNotFoundException e) {
	...
	public boolean saveOptions(OptionPanel optionPanel) {
		...
		if (handlePathToUpdates(optionPanel) == false) {
			optionPanel.textFieldPathToUpdates.setText(configFile.getPathToUpdates());
			FLAG = true;
		}	
		...
		if (configFile.pathToFibUpdateFiles.equals(optionPanel.textFieldPathToFibUpdateFiles.getText()) == false) {
			configFile.pathToFibUpdateFiles = optionPanel.textFieldPathToFibUpdateFiles.getText();
			needToRestart = true;
		}
		
		if (configFile.pathToFibReleaseFiles.equals(optionPanel.textFieldPathToFibReleaseFiles.getText()) == false) {
			configFile.pathToFibReleaseFiles = optionPanel.textFieldPathToFibReleaseFiles.getText();
			needToRestart = true;
		}
		
		if (configFile.pathToAssFiles.equals(optionPanel.textFieldPathToAssFiles.getText()) == false) {
			configFile.pathToAssFiles = optionPanel.textFieldPathToAssFiles.getText();
			needToRestart = true;
		}
		
		if (configFile.pathToJamFiles.equals(optionPanel.textFieldPathToJamFiles.getText()) == false) {
			configFile.pathToJamFiles = optionPanel.textFieldPathToJamFiles.getText();
			needToRestart = true;
		}
	...
	public OptionPanel getOptionPanel() {
		...
		optionPanel.textFieldPathToFibUpdateFiles.setText(configFile.pathToFibUpdateFiles);
		optionPanel.textFieldPathToFibReleaseFiles.setText(configFile.pathToFibReleaseFiles);
		optionPanel.textFieldPathToAssFiles.setText(configFile.pathToAssFiles);
		optionPanel.textFieldPathToJamFiles.setText(configFile.pathToJamFiles);
	...
	public void optionChooseFilePressed(OptionPanel optionPanel) {
		JFileChooser chooser = new JFileChooser();
		chooser.setCurrentDirectory(new File(configFile.getPathToUpdates()));
		chooser.setFileFilter(new FileNameExtensionFilter("(*.xml)", "xml"));
		if (chooser.showOpenDialog(controller.getParentForPositionOnly()) == JFileChooser.APPROVE_OPTION) {
			optionPanel.textFieldPathToUpdates.setText(chooser.getSelectedFile().getPath());
		};
	}
